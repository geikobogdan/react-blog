...
import { addItems } from "../redux/actions/galeryItems";
...
function AddForm({ path, activeUser, categories }) {
  ...
  const onSubmit = (e) => {
    ...
    if (path === "items") {
      ...
      data.authorU = toUpper(data.author);
  ...
  return (
    <div className="form">
      ...
      {submit && (
        ...
        >
          <Alert severity="success">{path === "items"? "Елемент": "Категорія"} додана успішно</Alert>
        </Snackbar>
      )}
    </div>
  );
